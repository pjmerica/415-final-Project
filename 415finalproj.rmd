...
newlife = newlife[-c(1,12,19)] #Getting rid of variables with collinearity & countries
...
##Creating Matrices
```{r}
trainmatrix = model.matrix(Life_Expectancy ~ Adult_Mortality + BMI + HIV.AIDS + Resource_Income + Schooling, data = trainlife)[,-1]
colm = colMeans(trainmatrix)
colsd = sqrt(diag(var(trainmatrix)))
trainmatrix = scale(trainmatrix, colm, colsd)
testmatrix = model.matrix(Life_Expectancy ~ Adult_Mortality + BMI + HIV.AIDS + Resource_Income + Schooling , data = testlife)[,-1]
